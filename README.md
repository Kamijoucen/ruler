...
Ruler 是一款轻量级、可嵌入的脚本引擎，适用于 Java 宿主环境。表达式与脚本共用统一的编译入口——单行表达式即只有一条语句的脚本，提供简洁的动态类型语法、闭包、数组与对象操作、中缀运算符扩展、代理以及规则块等特性。

![ruler shell](pic/shell.gif)

## 核心特性

- **统一编译入口**：`Ruler.compile(text, config)` 同时支持单行表达式与多行脚本，无需显式区分。
