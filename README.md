...
- **模式匹配（Match）**：支持基于字面量、变量绑定与通配符的 `match` 表达式。
...
支持的 pattern 类型：
- **字面量**：`1`、`"hello"`、`true`、`false`、`null`、`3.14`
- **变量绑定**：任意标识符，总是匹配，并将值绑定到该变量
- **通配符**：`_`，总是匹配，不绑定变量

匹配顺序从上到下，命中第一个符合条件的 case 即返回其表达式值。绑定的变量只在 `->` 右侧的表达式内有效，不会泄漏到外部作用域。body 可以是单行表达式，也可以是 `{ ... }` 块。

同一行内书写多个 case 时，需用分号分隔：
...
> 字面量匹配使用**严格相等**（`===`）语义，因此 `1` 会匹配 `1.0`，但不会匹配 `"1"`。
